...
#   origin: Khatchig Mouradian. Khatchig Mouradian is a journalist, writer and translator born in Lebanon .
...
#   origin: Jacob Henry Studer. Jacob Henry Studer (26 February 1840 Columbus, Ohio - 2 August 1904 New York City) was a printer, lithographer, painter, and popular
#   x: Jaco⁇enry ⁇b H⁇□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□
#   y: aco⁇enry ⁇b H⁇□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□
...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ### YOUR CODE HERE ###
        # 用传入的idx提取文件中的某一个句子出来
        entry = self.data[idx]
        # assert len(entry) >= 1  # 确保句子不会太短
        truncated_len = random.randint(4, int(self.block_size * 7 / 8))
        entry = entry[:truncated_len]

        # Now, break the (truncated) document into three substrings
        masked_content_length: int = round(random.gauss(mu=len(entry)/4, sigma=0.1))
        prefix_length: int = int((truncated_len - masked_content_length) / 2)

        prefix = entry[0: prefix_length]
        masked_content = entry[prefix_length: prefix_length + masked_content_length]
        suffix = entry[prefix_length + masked_content_length:]

        # Rearrange these substrings into the following form
        # [prefix] MASK_CHAR [suffix] MASK_CHAR [masked_content] [pads]

        # masked_string = prefix + list(self.stoi[self.MASK_CHAR]) + suffix + list(self.stoi[self.MASK_CHAR])
        # + masked_content + list(self.stoi[self.MASK_CHAR]) + list(self.stoi[self.PAD_CHAR]) * pad_length
        # 这样写是不可以的，因为self.stoi[self.MASK_CHAR]是一个整数（字典的值），不能用于list()函数，这个函数需要一个可迭代的对象
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content
        pad_length = self.block_size + 1 - len(masked_string)
        masked_string = masked_string + self.PAD_CHAR * pad_length

        # We now use masked_string to construct the input and output example pair.
        input_x = masked_string[:-1]
        output_y = masked_string[1:]

        # 把字符转换为索引
        input_x = torch.tensor([self.stoi[ch] for ch in input_x], dtype=torch.long)
        output_y = torch.tensor([self.stoi[ch] for ch in output_y], dtype=torch.long)
...
class NameDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 输入序列 x 包含掩码字符连接的输入和输出
        y = self.PAD_CHAR*(len(inp)-1) + x[len(inp):]
        # 目标序列 y 则以 PAD_CHAR 掩盖输入部分，避免模型对输入进行预测。
        # 假设 block_size = 12，输入的样本数据是 John\tSmith
        # x = "John‽Smith‽"，假设 block_size = 12，这就是最终的输入序列。
        # y = "□□‽Smith‽"，其中 □□ 是 PAD_CHAR 填充的前缀，用来避免对输入部分的预测。

        x = x[:-1]
        x = torch.tensor([self.stoi[c] for c in x], dtype=torch.long)
        y = torch.tensor([self.stoi[c] for c in y], dtype=torch.long)
        if len(x) != len(y):
            raise Exception("x和y的长度不相等")
        return x, y
